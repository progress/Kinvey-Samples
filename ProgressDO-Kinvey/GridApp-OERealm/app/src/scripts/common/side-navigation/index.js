...
const SideNavigationCtrl = function($scope, $element, $state, authService) {
    var SCREEN_MD = 1024;
    ...
    function removeElementsWithNoPermission(element) {
        ...
        const stateService = injector && injector.get('$state');

        const elements = element.find('> ul li');
        elements.each((index, element) => {
            let showElement = true;
            const stateName = angular.element(element).attr('data-state');
            const state = stateService && stateService.get(stateName);
            if (state) {
                // If one of the views in a module has a permissions for the moudule 
...
SideNavigationCtrl.$inject = ['$scope', '$element', '$state', 'authService'];
